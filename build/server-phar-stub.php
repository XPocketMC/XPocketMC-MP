...
namespace xpocketmc\server_phar_stub;
...
function preparePharCacheDirectory() : string{
	...
	$i = 0;
	do{
		$tmpPath = sys_get_temp_dir() . '/xpocketmc-MP-phar-cache.' . $i;
		$i++;
...
/**
 * Prepares a decompressed .tar of xpocketmc-MP.phar in the system temp directory for loading code from.
 *
...
echo "Preparing xpocketmc-MP.phar decompressed cache...\n";
$start = hrtime(true);
$cacheName = preparePharCache($tmpDir, __FILE__);
echo "Cache ready at $cacheName in " . number_format((hrtime(true) - $start) / 1e9, 2) . "s\n";

require 'phar://' . str_replace(DIRECTORY_SEPARATOR, '/', $cacheName) . '/src/xpocketmc.php';
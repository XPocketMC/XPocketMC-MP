...
- The following Composer dependency versions have changed (**PLEASE READ, dependency API changes are not mentioned in this changelog!**):
  - `xpocketmc/bedrock-protocol` has been added at [7.0.0](https://github.com/pmmp/BedrockProtocol/releases/tag/7.0.0+bedrock-1.18.0).
  - `xpocketmc/classloader` has been updated from 0.1.0 to [0.2.0](https://github.com/pmmp/ClassLoader/releases/tag/0.2.0) (**significant API changes, new features**).
  - `xpocketmc/color` has been added at [0.2.0](https://github.com/pmmp/Color/releases/tag/0.2.0).
  - `xpocketmc/errorhandler` has been added at [0.3.0](https://github.com/pmmp/ErrorHandler/releases/tag/0.3.0).
  - `xpocketmc/locale-data` has been added at [2.0.16](https://github.com/pmmp/Language/releases/tag/2.0.16).
  - `xpocketmc/log` has been updated from 0.2.0 to [0.4.0](https://github.com/pmmp/Log/releases/tag/0.4.0) (**minor API changes**, see also [0.3.0](https://github.com/pmmp/Log/releases/tag/0.3.0)).
  - `xpocketmc/nbt` has been updated from 0.2.18 to [0.3.0](https://github.com/pmmp/NBT/releases/tag/0.3.0) (**significant API changes**).
  - `xpocketmc/raklib` has been updated from 0.12.7 to [0.14.2](https://github.com/pmmp/RakLib/releases/tag/0.14.2) (**significant API changes**, see also [0.13.0](https://github.com/pmmp/RakLib/releases/tag/0.13.0)).
  - `xpocketmc/raklib-ipc` has been added at [0.1.0](https://github.com/pmmp/RakLibIpc/releases/tag/0.1.0) (components extracted from RakLib).
  - `xpocketmc/snooze` has been updated from 0.1.0 to [0.3.0](https://github.com/pmmp/Snooze/releases/tag/0.3.0) (**minor API changes**, see also [0.2.0](https://github.com/pmmp/Snooze/releases/tag/0.2.0)).
  - `xpocketmc/spl` has been dropped.
...
- The following submodules have been removed:
  - `resources/vanilla`: BedrockData is now included via Composer dependency [`xpocketmc/bedrock-data`](https://packagist.org/packages/xpocketmc/bedrock-data).
  - `resources/locale`: Language files are now included via Composer dependency [`xpocketmc/locale-data`](https://packagist.org/packages/xpocketmc/locale-data).
...
### Tools
Some new scripts have been added in the `tools/` directory of the repository. These scripts may use the xpocketmc-MP core library, but are intended to be run standalone.
...
### Internals
- The `xpocketmc` subdirectory has been removed from `src`. [PSR-4 autoloading is now used thanks to Composer](https://github.com/pmmp/xpocketmc-MP/blob/4.0.0/composer.json#L63).
...
- All network metadata related constants have been removed from the `Entity` class and moved to the protocol layer. It is intended to remove network metadata from the API entirely, but this has not yet been completed.
  - `Entity::DATA_FLAG_*` constants have been moved to `xpocketmc\network\mcpe\protocol\types\entity\EntityMetadataFlags`.
  - `Entity::DATA_TYPE_*` constants have been moved to `xpocketmc\network\mcpe\protocol\types\entity\EntityMetadataTypes`.
  - `Entity::DATA_*` constants have been moved to `xpocketmc\network\mcpe\protocol\types\entity\EntityMetadataProperties`.
- `DataPropertyManager` has been moved to the `xpocketmc\network\mcpe\protocol\types\entity` namespace, and as such isn't considered part of the API anymore.
...
- The boundaries between MCPE enchantment IDs and xpocketmc-MP internals are now more clear.
  - ID handling is moved to `xpocketmc\data\bedrock\EnchantmentIdMap` singleton.
...
### Player
- The following classes have been added/moved to the new `xpocketmc\player` namespace:
  - `Achievement`
  - `GameMode`
  - `IPlayer`
  - `OfflinePlayer`
...
- The following API methods have been removed:
  ...
  - `Server->getGamemodeString()` - replaced by `xpocketmc\player\GameMode->getTranslationKey()`
  - `Server->getGamemodeName()` - replaced by `xpocketmc\player\GameMode->name()`
...
- `xpocketmc\world\particle\Particle` no longer extends `xpocketmc\math\Vector3`, and has been converted to an interface.
- Added the following `Particle` classes:
  - `DragonEggTeleportParticle`
  - `PunchBlockParticle`

#### Sounds
- `xpocketmc\world\sound\Sound` no longer extends `xpocketmc\math\Vector3`, and has been converted to an interface.
- `Sound->encode()` now accepts `xpocketmc\math\Vector3`.
...
- Fixed all chunks containing furnaces being treated as corrupted in worlds older than 2017.
  - This was caused by a strict corruption check detecting bad data created by a bug in xpocketmc-MP that was fixed in 2017.
